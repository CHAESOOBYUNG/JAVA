...
public class AppMain extends Application {
	...
	public void start(Stage primaryStage) throws Exception {
		VBox root = new VBox(); //Present 하위 객체인 VBox 생성
		root.setPrefWidth(350); //VBox의 폭 설정
		root.setPrefHeight(150); //VBox의 높이 설정
		root.setAlignment(Pos.CENTER); //컨드롤을 중앙으로 정렬
		root.setSpacing(20); //컨트롤을 수직 간격
		...
		Scene scene = new Scene(root); //VBox를 루트 커테이너로 해서 Scene 생성
